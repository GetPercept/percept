...
async def _send_imessage(text: str):
    ...
    try:
        ...
        proc = await asyncio.create_subprocess_exec(
            "/opt/homebrew/bin/imsg", "send", "--to", "+10000000000", "--text", text,
...
def _normalize_spoken_email(text: str) -> str:
    """Convert spoken email: 'david at vectorcare dot com' → 'user@example.com'"""
    t = text.lower().strip()
...
async def _flush_transcript(session_key: str):
    ...
    if has_wake_word:
        try:
            ...
            env = {**os.environ, "PATH": "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin"}
            proc = await asyncio.create_subprocess_exec(
                "/opt/homebrew/bin/openclaw", "agent", "--message", msg, "--to", "+10000000000",
                stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
                env=env,
            )
...
{full_transcript[-3000:]}"""
    ...
    try:
        ...
        proc_summary = await asyncio.create_subprocess_exec(
            "/opt/homebrew/bin/openclaw", "agent", "--message", summary_prompt, "--to", "+10000000000", "--json",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
            env=env,
        )
...
Question: {text}"""
    ...
    try:
        env = {**os.environ, "PATH": "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin"}
        proc = await asyncio.create_subprocess_exec(
            "/opt/homebrew/bin/openclaw", "agent", "--message", prompt, "--to", "+10000000000",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
            env=env,
        )
